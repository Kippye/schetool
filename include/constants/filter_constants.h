...
namespace filter_consts {
    ...
    const std::map<LogicalOperatorEnum, const char*> logicalOperatorStrings = {{LogicalOperatorEnum::And, "AND"},
    ...
    struct TypeComparisonInfo {
        public:
            ...
            TypeComparisonInfo(const std::vector<Comparison>& _comparisons, const std::vector<std::string>& _names)
                : comparisons(_comparisons), names(_names) {
            }
    };
    ...
    const std::map<SCHEDULE_TYPE, std::vector<Comparison>> typeComparisons = {
        ...
        {SCH_MULTISELECT,
         {Comparison::Is, Comparison::IsNot, Comparison::IsEmpty, Comparison::Contains, Comparison::DoesNotContain}},
        {SCH_WEEKDAY,
         {Comparison::Is,
          Comparison::IsNot,
          Comparison::IsEmpty,
