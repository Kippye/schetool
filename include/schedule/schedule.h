...
class Schedule {
    private:
        ...
        std::function<void(size_t, SelectOptionsModification)> modifyColumnSelectOptionsListener =
            [&](size_t i, SelectOptionsModification modification) { modifyColumnSelectOptions(i, modification); };
        ...
        std::function<void(size_t, size_t, std::string)> setFilterGroupNameListener =
            [&](size_t col, size_t groupIndex, std::string name) { setColumnFilterGroupName(col, groupIndex, name); };
        std::function<void(size_t, size_t, LogicalOperatorEnum)> setFilterGroupOperatorListener =
            [&](size_t col, size_t groupIndex, LogicalOperatorEnum logicalOperator) {
                setColumnFilterGroupOperator(col, groupIndex, logicalOperator);
            };
        std::function<void(size_t, size_t, bool)> setFilterGroupEnabledListener =
            [&](size_t col, size_t groupIndex, bool enabled) { setColumnFilterGroupEnabled(col, groupIndex, enabled); };
        ...
        std::function<void(size_t, size_t, size_t, LogicalOperatorEnum)> setFilterOperatorListener =
            [&](size_t col, size_t groupIndex, size_t filterIndex, LogicalOperatorEnum logicalOperator) {
                setColumnFilterOperator(col, groupIndex, filterIndex, logicalOperator);
            };
        std::function<void(size_t, size_t, size_t)> removeFilterListener =
            [&](size_t col, size_t groupIndex, size_t filterIndex) { removeColumnFilter(col, groupIndex, filterIndex); };

        std::function<void(size_t, size_t, size_t, FilterRuleContainer)> addFilterRuleListener = [&](size_t col,
                                                                                                     size_t groupIndex,
                                                                                                     size_t filterIndex,
                                                                                                     FilterRuleContainer
                                                                                                         filterRule) {
            ...
            switch (columnType) {
                ...
                case SCH_TEXT:
                    ...
                    break;
                case SCH_SELECT:
                    addColumnFilterRule<SingleSelectContainer>(
                        col, groupIndex, filterIndex, filterRule.getAsType<SingleSelectContainer>());
                    break;
                case SCH_MULTISELECT:
                    addColumnFilterRule<SelectContainer>(col, groupIndex, filterIndex, filterRule.getAsType<SelectContainer>());
                    break;
                case SCH_WEEKDAY:
                    addColumnFilterRule<WeekdayContainer>(
                        col, groupIndex, filterIndex, filterRule.getAsType<WeekdayContainer>());
                    break;
                case SCH_TIME:
                ...
                default:
                    printf(
                        "Schedule listener addFilterRuleListener(): Failed to find Schedule::addColumnFilterRule() for type %d\n",
                        columnType);
                    break;
            }
        };
        std::function<void(size_t, size_t, size_t, size_t, FilterRuleContainer, FilterRuleContainer)> editFilterRuleListener =
            [&](size_t col,
                size_t groupIndex,
                size_t filterIndex,
                size_t ruleIndex,
                ...
                switch (columnType) {
                    case SCH_BOOL:
                        replaceColumnFilterRule<bool>(
                            col, groupIndex, filterIndex, ruleIndex, previousRule.getAsType<bool>(), rule.getAsType<bool>());
                        break;
                    case SCH_NUMBER:
                        replaceColumnFilterRule<int>(
                            col, groupIndex, filterIndex, ruleIndex, previousRule.getAsType<int>(), rule.getAsType<int>());
                        break;
                    case SCH_DECIMAL:
                        replaceColumnFilterRule<double>(col,
                                                        groupIndex,
                                                        filterIndex,
                                                        ruleIndex,
                                                        previousRule.getAsType<double>(),
                                                        rule.getAsType<double>());
                        break;
                    case SCH_TEXT:
                        replaceColumnFilterRule<std::string>(col,
                                                             groupIndex,
                                                             filterIndex,
                                                             ruleIndex,
                                                             previousRule.getAsType<std::string>(),
                                                             rule.getAsType<std::string>());
                        break;
                    case SCH_SELECT:
                        replaceColumnFilterRule<SingleSelectContainer>(col,
                                                                       groupIndex,
                                                                       filterIndex,
                                                                       ruleIndex,
                                                                       previousRule.getAsType<SingleSelectContainer>(),
                                                                       rule.getAsType<SingleSelectContainer>());
                        break;
                    case SCH_MULTISELECT:
                        replaceColumnFilterRule<SelectContainer>(col,
                                                                 groupIndex,
                                                                 filterIndex,
                                                                 ruleIndex,
                                                                 previousRule.getAsType<SelectContainer>(),
                                                                 rule.getAsType<SelectContainer>());
                        break;
                    case SCH_WEEKDAY:
                        replaceColumnFilterRule<WeekdayContainer>(col,
                                                                  groupIndex,
                                                                  filterIndex,
                                                                  ruleIndex,
                                                                  previousRule.getAsType<WeekdayContainer>(),
                                                                  rule.getAsType<WeekdayContainer>());
                        break;
                    case SCH_TIME:
                        replaceColumnFilterRule<TimeContainer>(col,
                                                               groupIndex,
                                                               filterIndex,
                                                               ruleIndex,
                                                               previousRule.getAsType<TimeContainer>(),
                                                               rule.getAsType<TimeContainer>());
                        break;
                    case SCH_DATE:
                        replaceColumnFilterRule<DateContainer>(col,
                                                               groupIndex,
                                                               filterIndex,
                                                               ruleIndex,
                                                               previousRule.getAsType<DateContainer>(),
                                                               rule.getAsType<DateContainer>());
                        break;
                    default:
                        printf(
                            "Schedule listener editFilterRuleListener(): Failed to find Schedule::replaceColumnFilterRule() for type %d\n",
                            columnType);
                        break;
                }
            };
        std::function<void(size_t, size_t, size_t, size_t)> removeFilterRuleListener = [&](size_t col,
                                                                                           size_t groupIndex,
                                                                                           size_t filterIndex,
                                                                                           size_t ruleIndex) {
            SCHEDULE_TYPE columnType = getColumn(col)->type;
            switch (columnType) {
                ...
                default:
                    printf(
                        "Schedule listener removeFilterRuleListener(): Failed to find Schedule::removeColumnFilterRule() for type %d\n",
                        columnType);
                    break;
            }
        };
        ...
        std::function<void(size_t, size_t, TimeContainer)> setElementValueListenerTime =
            [&](size_t col, size_t row, TimeContainer val) { setElementValue(col, row, val); };
        std::function<void(size_t, size_t, DateContainer)> setElementValueListenerDate =
            [&](size_t col, size_t row, DateContainer val) { setElementValue(col, row, val); };
        // column add / remove
        ...
        // row modification
        std::function<void(size_t)> addRowListener = [&](size_t row) { addRow(row); };
        std::function<void(size_t)> removeRowListener = [&](size_t row) { removeRow(row); };
        std::function<void(size_t)> duplicateRowListener = [&](size_t row) { duplicateRow(row); };
    ...
    public:
        ...
        void setColumnFilterGroupOperator(size_t column,
                                          size_t groupIndex,
        ...
        void setColumnFilterOperator(size_t column,
                                     size_t groupIndex,
                                     size_t filterIndex,
                                     LogicalOperatorEnum logicalOperator,
                                     bool addToHistory = true);
        ...
        template <typename T>
        void addColumnFilterRule(
            size_t column, size_t groupIndex, size_t filterIndex, FilterRule<T> filterRule, bool addToHistory = true) {
        ...
        template <typename T>
        void replaceColumnFilterRule(size_t column,
                                     size_t groupIndex,
                                     size_t filterIndex,
                                     size_t ruleIndex,
                                     FilterRule<T> previousFilterRule,
                                     FilterRule<T> filterRule,
                                     bool addToHistory = true) {
            if (m_core.replaceColumnFilterRule<T>(column, groupIndex, filterIndex, ruleIndex, filterRule)) {
                if (addToHistory) {
                    m_editHistory.addEdit<FilterRuleChangeEdit<T>>(
                        column, groupIndex, filterIndex, ruleIndex, previousFilterRule, filterRule);
                }
            }
        }
        template <typename T>
        void removeColumnFilterRule(
            ...
            if (m_core.removeColumnFilterRule(column, groupIndex, filterIndex, ruleIndex)) {
                if (addToHistory) {
                    m_editHistory.addEdit<FilterRuleAddOrRemoveEdit<T>>(
                        true, column, groupIndex, filterIndex, ruleIndex, filterRule);
                }
            }
        }
