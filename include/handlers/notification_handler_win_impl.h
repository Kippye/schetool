...
{
public:
    static Event<size_t> notificationActivatedEvent;
        ...
        ULONG dataCount) override
    {
        ...
        else
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                {
                    hr = S_OK;
                    std::wcout << L"Complete previous item activated. Notification ID: " << arguments.back() << std::endl;
                }
                // Clicked "Dismiss"
        ...
        "<toast launch='action=viewItem&amp;itemName={}' duration='{}'>"
            "<visual>"
                "<binding template='ToastGeneric'>"
                    ...
                    "<text>{} - {}</text>"
                    "<progress title='Schedule progress' value='0.6' valueStringOverride='15/26 items' status='{}'/>"
                "</binding>"
            "</visual>"
            "<actions>"
