...
DateWrapper::DateWrapper(const year_month_day& date)
    : DateWrapper((int)date.year(), (unsigned int)date.month(), (unsigned int)date.day()) {
}
...
std::time_t TimeWrapper::time_to_utc_time_t(const DateWrapper& date, const ClockTimeWrapper& time) {
    ...
    return system_clock::to_time_t(sys_days{ymd} + hours{time.getHours()} + minutes{time.getMinutes()});
}
