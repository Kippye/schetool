    ...
    switch (modification.m_type) {
        case (OPTION_MODIFICATION_ADD): {
            if (modification.m_options.has_value() == false) {
                std::cout << "SelectOptions::applyModification: Failed to add Select option because no options were provided"
                          << std::endl;
...
bool SelectOptions::removeOption(size_t option) {
    if (option < m_options.size() == false) {
        return false;
    }
...
bool SelectOptions::moveOption(size_t firstIndex, size_t secondIndex) {
    if (firstIndex < m_options.size() == false || secondIndex < m_options.size() == false) {
        return false;
    }
