...
FilterRuleEditorSubGui::FilterRuleEditorSubGui(const char* ID,
                                               const ScheduleCore& scheduleCore,
                                               ScheduleEvents& scheduleEvents,
                                               FilterGroupEditorState& filterGroupState)
    : Gui(ID), m_scheduleCore(scheduleCore), m_filterGroupState(filterGroupState) {
...
void FilterRuleEditorSubGui::draw(Window& window, Input& input, GuiTextures& guiTextures) {
                                ...
                                ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        auto invokeEditFilterRuleEvent = [&](const FilterRuleContainer& prevFilterRule) {
            editColumnFilterRule.invoke(m_filterGroupState.getColumnIndex(),
                                        m_filterGroupState.getFilterGroupIndex(),
                                        filterIndex,
                                        filterRuleIndex,
                                        prevFilterRule,
                                        m_filterRuleState.getFilterRule());
        };
        ...
        switch (m_filterRuleState.getType()) {
            ...
            case (SCH_TEXT): {
                ...
                if (ImGui::InputTextMultiline("##filterRuleEditor",
                                              buf,
                                              value.capacity(),
                                              ImVec2(0, 0),
            ...
            case (SCH_SELECT): {
                ...
                if (newComparison != Comparison::IsEmpty) {
                    ...
                    const std::vector<SelectOption>& options =
                        m_scheduleCore.getColumnSelectOptions(m_filterGroupState.getColumnIndex()).getOptions();
                    // Options
                    for (size_t i = 0; i < options.size(); i++) {
                        ...
                        auto setCurrentOptionSelected = [&](bool newSelected) {
                            ...
                            if (m_editing == true) {
                                filter.replaceRule(filterRuleIndex,
                                                   m_filterRuleState.getFilterRule().getAsType<SingleSelectContainer>());
                                invokeEditFilterRuleEvent(FilterRuleContainer(prevFilter));
                            }
                        };
                ...
                if (newComparison != Comparison::IsEmpty) {
                    ...
                    for (size_t i = 0; i < options.size(); i++) {
                        ...
                        auto setCurrentOptionSelected = [&](bool newSelected) {
                            ...
                            if (m_editing == true) {
                                filter.replaceRule(filterRuleIndex,
                                                   m_filterRuleState.getFilterRule().getAsType<SelectContainer>());
                                invokeEditFilterRuleEvent(FilterRuleContainer(prevFilter));
                            }
                        };
                ...
                if (newComparison != Comparison::IsEmpty && newComparison != Comparison::ContainsToday) {
                    ...
                    for (size_t i = 0; i < optionNames.size(); i++) {
                        ...
                        auto setCurrentOptionSelected = [&](bool newSelected) {
                            ...
                            if (m_editing == true) {
                                filter.replaceRule(filterRuleIndex,
                                                   m_filterRuleState.getFilterRule().getAsType<WeekdayContainer>());
                                invokeEditFilterRuleEvent(FilterRuleContainer(prevFilter));
                            }
                        };
        ...
        if (m_editing == false && ImGui::Button("Create")) {
            ...
            addColumnFilterRule.invoke(m_filterGroupState.getColumnIndex(),
                                       m_filterGroupState.getFilterGroupIndex(),
                                       filterIndex,
                                       m_filterRuleState.getFilterRule());
...
void FilterRuleEditorSubGui::openEdit(
    SCHEDULE_TYPE type, const std::string& columnName, size_t filterIndex, size_t ruleIndex, const ImRect& avoidRect) {
    if (m_filterGroupState.getFilterGroup().hasFilterAt(filterIndex) == false) {
        return;
    }
    if (ruleIndex < m_filterGroupState.getFilterGroup().getFilter(filterIndex).getRuleCount() == false) {
        return;
    }
    ...
    switch (type) {
        case (SCH_BOOL):
            m_filterRuleState.setup(
                type,
                filterIndex,
                ruleIndex,
                m_filterGroupState.getFilterGroup().getFilter(filterIndex).getRule(ruleIndex).getAsType<bool>());
            break;
        case (SCH_NUMBER):
            m_filterRuleState.setup(
                type,
                filterIndex,
                ruleIndex,
                m_filterGroupState.getFilterGroup().getFilter(filterIndex).getRule(ruleIndex).getAsType<int>());
            break;
        case (SCH_DECIMAL):
            m_filterRuleState.setup(
                type,
                filterIndex,
                ruleIndex,
                m_filterGroupState.getFilterGroup().getFilter(filterIndex).getRule(ruleIndex).getAsType<double>());
            break;
        case (SCH_TEXT):
            m_filterRuleState.setup(
                type,
                filterIndex,
                ruleIndex,
                m_filterGroupState.getFilterGroup().getFilter(filterIndex).getRule(ruleIndex).getAsType<std::string>());
            break;
        case (SCH_SELECT):
            m_filterRuleState.setup(type,
                                    filterIndex,
                                    ruleIndex,
                                    m_filterGroupState.getFilterGroup()
                                        .getFilter(filterIndex)
                                        .getRule(ruleIndex)
                                        .getAsType<SingleSelectContainer>());
            break;
        case (SCH_MULTISELECT):
            m_filterRuleState.setup(
                type,
                filterIndex,
                ruleIndex,
                m_filterGroupState.getFilterGroup().getFilter(filterIndex).getRule(ruleIndex).getAsType<SelectContainer>());
            break;
        case (SCH_WEEKDAY):
            m_filterRuleState.setup(
                type,
                filterIndex,
                ruleIndex,
                m_filterGroupState.getFilterGroup().getFilter(filterIndex).getRule(ruleIndex).getAsType<WeekdayContainer>());
            break;
        case (SCH_TIME):
            m_filterRuleState.setup(
                type,
                filterIndex,
                ruleIndex,
                m_filterGroupState.getFilterGroup().getFilter(filterIndex).getRule(ruleIndex).getAsType<TimeContainer>());
            break;
        case (SCH_DATE):
            m_filterRuleState.setup(
                type,
                filterIndex,
                ruleIndex,
                m_filterGroupState.getFilterGroup().getFilter(filterIndex).getRule(ruleIndex).getAsType<DateContainer>());
            break;
        default: {
...
void FilterRuleEditorSubGui::openCreate(SCHEDULE_TYPE type,
                                        const std::string& columnName,
                                        size_t filterIndex,
                                        const ImRect& avoidRect) {
    if (m_filterGroupState.getFilterGroup().hasFilterAt(filterIndex) == false) {
        return;
    }
    ...
    switch (type) {
        ...
        case (SCH_SELECT): {
            m_filterRuleState.setup(
                type, filterIndex, filter.getRuleCount(), FilterRule<SingleSelectContainer>(SingleSelectContainer()));
            break;
        }
        case (SCH_MULTISELECT): {
...
FilterEditorSubGui::FilterEditorSubGui(const char* ID, const ScheduleCore& scheduleCore, ScheduleEvents& scheduleEvents)
...
void FilterEditorSubGui::draw(Window& window, Input& input, GuiTextures& guiTextures) {
                                ...
                                ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        if (ImGui::Checkbox("##FilterGroupEnabled", &groupEnabled)) {
            ...
            setColumnFilterGroupEnabled.invoke(
                m_filterGroupState.getColumnIndex(), m_filterGroupState.getFilterGroupIndex(), groupEnabled);
        }
        ...
        if (ImGui::IsItemDeactivatedAfterEdit()) {
            setColumnFilterGroupName.invoke(m_filterGroupState.getColumnIndex(),
                                            m_filterGroupState.getFilterGroupIndex(),
                                            m_filterGroupState.getFilterGroup().getName());
        }
        ...
        auto drawFilterRule = [&](size_t filterIndex, size_t ruleIndex) {
            ...
            ImVec2 ruleButtonSize =
                ImVec2(std::min(ruleTextSize.x + ImGui::GetStyle().ItemInnerSpacing.x * 2,
                                                       ...
                                                       ImVec2(removeRuleButtonSize, removeRuleButtonSize)))
            {
                ...
                removeColumnFilterRule.invoke(
                    m_filterGroupState.getColumnIndex(), m_filterGroupState.getFilterGroupIndex(), filterIndex, ruleIndex);
            }
        };
        ...
        for (size_t f = 0; f < m_filterGroupState.getFilterGroup().getFilters().size(); f++) {
            ...
            ImGui::SetNextWindowSizeConstraints(ImVec2(0.0f, 0.0f),
                                                ImVec2(FLT_MAX, /* Calculate size using number of FilterRules */ FLT_MAX));
            if (ImGui::BeginChild(std::string("###Filter").append(std::to_string(f)).c_str(),
                                  ImVec2(-FLT_MIN, 0.0f),
