...
bool PreferencesIO::writePreferences()
{
    if (fs::exists(m_dataDir) == false)
    {
        ...
        {
            std::cout << std::format("PreferencesIO::writePreferences(): Data directory did not exist. Created a data directory at: '{}'", m_dataDir.string()) << std::endl;
        }
        else
        {
            std::cout << std::format("PreferencesIO::writePreferences(): Data directory did not exist. FAILED to create a data directory at: '{}'", m_dataDir.string()) << std::endl;
            return false;
        }
    }
...
bool PreferencesIO::readPreferences()
{
    ...
    {
        std::cout << std::format("PreferencesIO::readPreferences(): Tried to read preferences at path to non-existant file: '{}'", preferencesFilePath.string()) << std::endl;
        return false;
    }
    if (m_converter.isValidPreferencesFile(preferencesFilePath.string().c_str()) == false)
    {
        std::cout << std::format("PreferencesIO::readPreferences(): Tried to read invalid preferences file at path: '{}'", preferencesFilePath.string()) << std::endl;
        return false;
    }
