...
bool ScheduleIO::applyAutosaveToFile(const char* fileName)
{
    ...
    {
        std::cout << std::format("ScheduleIO::applyAutosaveToFile(): Base file for Schedule '{}' not found at path: '{}'", fileName, pathToFile.string()) << std::endl;
        return false;
    }
    if (fs::exists(pathToAutosaveFile) == false)
    {
        std::cout << std::format("ScheduleIO::applyAutosaveToFile(): Autosave file for Schedule '{}' not found at path: '{}'", fileName, pathToAutosaveFile.string()) << std::endl;
        return false;
    }
    ...
    try
    {
        if (fs::copy_file(pathToAutosaveFile, pathToFile, fs::copy_options::update_existing))
        {
            ...
            passFileNamesToGui();
            std::cout << std::format("ScheduleIO::applyAutosaveToFile(): Applied autosave to Schedule '{}'", fileName) << std::endl;
            return true;
        }
    }
    // Failed to copy
    ...
    {
        std::cout << std::format("ScheduleIO::applyAutosaveToFile(): Could not copy autosave at '{}' to file at '{}'. Error: {}", pathToAutosaveFile.string(), pathToFile.string(), e.what()) << std::endl;
        return false;
    }
    passFileNamesToGui();
...
bool ScheduleIO::writeSchedule(const char* name)
{
    if (fs::exists(m_saveDir) == false)
    {
        ...
        {
            std::cout << std::format("ScheduleIO::writeSchedule(): Schedule save directory did not exist. Created a 'schedules' directory at: '{}'", m_saveDir.string()) << std::endl;
        }
        else
        {
            std::cout << std::format("ScheduleIO::writeSchedule(): Schedule save directory did not exist. FAILED to create a 'schedules' directory at: '{}'", m_saveDir.string()) << std::endl;
            return false;
        }
    }
...
{    
    ...
    {
        std::cout << std::format("ScheduleIO::readSchedule(): Tried to read Schedule at path to non-existant file: '{}'", schedulePath.string()) << std::endl;
        return false;
    }
    ...
    if (std::optional<FileInfo> readFileInfo = m_converter.readSchedule(schedulePath.string().c_str(), m_schedule.getAllColumnsMutable()))
    {
        ...
        std::cout << std::format("ScheduleIO::readSchedule(): Read Schedule from file: '{}'", schedulePath.string()) << std::endl;
    ...
    {
        std::cout << std::format("ScheduleIO::readSchedule(): Failed when reading Schedule from file: '{}'", schedulePath.string()) << std::endl;
        return false;
    }
}
...
bool ScheduleIO::deleteSchedule(const char* name)
{
    ...
    if (fs::exists(schedulePath) == false)
    {
        std::cout << std::format("ScheduleIO::deleteSchedule(): Tried to delete non-existent Schedule at: '{}'", schedulePath.string()) << std::endl;
        return false;
    }
...
bool ScheduleIO::renameCurrentFile(const std::string& newName)
{
    ...
    if (fs::exists(m_saveDir) == false)
    {
        if (fs::create_directories(m_saveDir))
        {
            std::cout << std::format("ScheduleIO::renameCurrentFile(): Tried to rename a Schedule but the schedules directory did not exist. Created a schedules directory at: '{}'", m_saveDir.string()) << std::endl;
            schedulesDirWasCreated = true;
        }
        else
        {
            std::cout << std::format("ScheduleIO::renameCurrentFile(): Tried to rename a Schedule but the schedules directory did not exist. FAILED to create a schedules directory at: '{}'", m_saveDir.string()) << std::endl;
        }
    }
    ...
    {
        std::cout << std::format("ScheduleIO::renameCurrentFile(): Tried to change the name of the Schedule file, but the file was not found at its previous path: '{}'", pathToOpenFile.string()) << std::endl;
        writeSchedule(newName.c_str());
        std::cout << std::format("ScheduleIO::renameCurrentFile(): Wrote current file to renamed path: '{}'", pathToRenamedFile.string()) << std::endl;
    }
...
{
    if (fs::exists(path) == false) { std::cout << std::format("ScheduleIO::getFileEditTime(): No file exists at path: '{}'", path.string()) << std::endl; return 0; }
...
{
    if (fs::exists(path) == false) { std::cout << std::format("ScheduleIO::getFileEditTimeWrapped(): No file exists at path: '{}'", path.string()) << std::endl; return TimeWrapper(); }
...
{
    if (fs::exists(path) == false) { std::cout << std::format("ScheduleIO::getFileEditTimeString(): No file exists at path: '{}'", path.string()) << std::endl; return std::string(""); }
...
std::vector<std::string> ScheduleIO::getScheduleStemNames(bool includeAutosaves)
{
    ...
    if (fs::exists(m_saveDir) == false)
    {
        std::cout << std::format("ScheduleIO::getScheduleStemNames(): Schedules directory '{}' does not exist.", m_saveDir.string()) << std::endl;
        return filenames;
    }
    ...
    for (const auto& entry: fs::directory_iterator(m_saveDir))
    {
        ...
        {
            std::cout << "ScheduleIO::getScheduleStemNames(): Skipped file " << entry.path().string() << " (entry.is_regular_file() == false)" << std::endl;
            continue;
        }
        ...
        {
            std::cout << "ScheduleIO::getScheduleStemNames(): Skipped file " << entry.path().string() << " (not a valid schedule file)" << std::endl;
            continue;
        }
...
std::vector<std::string> ScheduleIO::getScheduleStemNamesSortedByEditTime(bool includeAutosaves)
{
    ...
    if (fs::exists(m_saveDir) == false)
    {
        std::cout << std::format("ScheduleIO::getScheduleStemNamesSortedByEditTime(): Schedules directory '{}' does not exist.", m_saveDir.string()) << std::endl;
        return filenames;
    }
    ...
    for (const auto& entry: fs::directory_iterator(m_saveDir))
    {
        ...
        if (entry.is_regular_file() == false)
        {
            std::cout << "ScheduleIO::getScheduleStemNamesSortedByEditTime(): Skipped file " << entry.path().string() << " (entry.is_regular_file() == false)" << std::endl;
            continue;
        }
        // Skip non-schedule files
        if (isScheduleFilePath(entry.path()) == false)
        ...
        if (isValidScheduleFile(entry.path()) == false)
        {
            std::cout << "ScheduleIO::getScheduleStemNamesSortedByEditTime(): Skipped file " << entry.path().string() << " (not a valid schedule file)" << std::endl;
            continue;
        }
        // Skip autosave files if includeAutosaves == false
        if (includeAutosaves == false && isAutosave(entry.path().stem().string()))
...
std::string ScheduleIO::getLastEditedScheduleStemName()
{
    if (fs::exists(m_saveDir) == false)
    {
        std::cout << std::format("ScheduleIO::getLastEditedScheduleStemName(): Schedules directory '{}' does not exist.", m_saveDir.string()) << std::endl;
        return std::string("");
    }
    ...
    for (const auto& entry: dirIterator)
    {
        ...
        if (entry.is_regular_file() == false)
        {
            std::cout << "ScheduleIO::getLastEditedScheduleStemName(): Skipped file " << entry.path().string() << " (entry.is_regular_file() == false)" << std::endl;
            continue;
        }
        // Skip non-schedule files
        if (isScheduleFilePath(entry.path()) == false)
        ...
        if (isValidScheduleFile(entry.path()) == false)
        {
            std::cout << "ScheduleIO::getLastEditedScheduleStemName(): Skipped file " << entry.path().string() << " (not a valid schedule file)" << std::endl;
            continue;
        }
